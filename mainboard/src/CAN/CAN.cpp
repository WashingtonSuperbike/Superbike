...
// Each of these functions deciphers a specific CAN message and updates the corresponding struct in bike context
static void decipherEVCCStats(twai_message_t msg, ChargeControllerStats *evcc_stats);
static void decipherChargerStats(twai_message_t msg, ChargerStats *charger_stats);
static void decodeMotorStats(twai_message_t msg, MotorStats *motor_stats);
static void decodeMotorTemps(twai_message_t msg, MotorTemps *motor_temps);
static void decipherBMSStatus(twai_message_t msg, BMSStatus *bms_status);
static void decipherCellsVoltage(twai_message_t msg, BatteryVoltages *battery_voltages);
static void decipherThermistors(twai_message_t msg, ThermistorTemps *thermistor_temps);
...
static void printMessage(twai_message_t msg);

// unused currently but should be implemented into the current firmware
static void printBMSStatus(BMSStatus bms_status);
static void requestCellVoltages();
...
static void requestCellVoltages()
{
    ...
    next_can_id = next_can_id == BMSC1_LTC1_REQUEST_CELLS ? BMSC1_LTC2_REQUEST_CELLS : BMSC1_LTC1_REQUEST_CELLS;
}