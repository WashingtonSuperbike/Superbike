...
// ============================================================================
// These were moved from Display.h to Display.cpp

Adafruit_ILI9341 tft = Adafruit_ILI9341(TFT_CS, TFT_DC, TFT_RST);

PrintedDataStruct printedVals[NUM_DATA];

PrintedDataStruct *batteryVoltage = &printedVals[0];
PrintedDataStruct *motorControllerVoltage = &printedVals[1];
PrintedDataStruct *auxBatteryVoltage = &printedVals[2];
PrintedDataStruct *rpm = &printedVals[3];
PrintedDataStruct *motorTemperature = &printedVals[4];
PrintedDataStruct *motorCurr = &printedVals[5];
PrintedDataStruct *errMessage = &printedVals[6];
PrintedDataStruct *chargerVolt = &printedVals[7];
PrintedDataStruct *chargerCurr = &printedVals[8];
PrintedDataStruct *bmsStatusFlag = &printedVals[9];
PrintedDataStruct *evccVolt = &printedVals[10];

PrintedDataThermStruct thermiData;
PrintedDataTimeStruct timeData;
...
void displayTask(void *displayTaskData) {
  while (1) {
    superbike::Context *context = (superbike::Context *)displayTaskData;
...
float aux_voltage_read() {
  /// Pin may need to change once configuration is determined.
